...
"<div id='file-label'>?? Klik untuk memilih file (.bin)</div>"
...
"  xhr.onload = function() {"
"    if (xhr.status == 200) {"
"      document.getElementById('status-text').innerText = '? Flashing Sukses! Me-reboot ESP32 dalam 5 detik...';"
"      setTimeout(function(){ location.reload(); }, 6000);"
"    } else {"
"      document.getElementById('status-text').innerText = '? Gagal update: ' + xhr.responseText;"
"      btn.disabled = false;"
"    }"
"  };"
"  xhr.onerror = function() {"
"    document.getElementById('status-text').innerText = '? Error jaringan saat upload.';"
"    btn.disabled = false;"
"  };"
